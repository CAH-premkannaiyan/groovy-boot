package com.cardinal.fuse.healthsense.config.locale
...
class AngularCookieLocaleResolver extends CookieLocaleResolver {
	...
	Locale resolveLocale(HttpServletRequest request) {
		...
		(Locale) request.getAttribute(LOCALE_REQUEST_ATTRIBUTE_NAME)
	}

	@Override
	LocaleContext resolveLocaleContext(final HttpServletRequest request) {
		parseLocaleCookieIfNecessary(request)
